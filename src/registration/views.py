...
"""
from settings import SSL_MEDIA_URL
...
def register(request, backend, success_url=SSL_MEDIA_URL + '/confreg/need_to_activate', form_class=None,
    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_user,key = backend.register(request, **form.cleaned_data)
            if success_url is None:
                to, args, kwargs = backend.post_registration_redirect(request, new_user,key)
                return redirect(to, *args, **kwargs)
            else:
                context = RequestContext(request)
                return render_to_response('confreg/need_to_activate.html',
                  {'form': form, 'SSL_MEDIA_URL': SSL_MEDIA_URL,'key':key},
                  context_instance=context)
    else:
        form = form_class()
    ...
    return render_to_response(template_name,
                              {'form': form, 'SSL_MEDIA_URL': SSL_MEDIA_URL},
                              context_instance=context)
